      ...
      <div className="hidden md:grid md:grid-cols-4 lg:grid-cols-7 gap-3">
        {metrics.map((stat, i) => (
          <motion.div 
            ...
            className={cn(
              "bg-card border border-cb-border shadow-cb-level-1 p-4 flex flex-col gap-3 group hover:border-primary/20 transition-all cursor-default",
      ...
      <div className={cn(
        ...
        <div className="space-y-4">
            ...
            <AnimatePresence mode="popLayout">
              {devoluciones.map((dev, idx) => (
                <motion.div
                  ...
                  className={cn(
                    "bg-card border p-4 space-y-4 active:scale-[0.98] transition-all",
                    selectedTickets.has(String(dev.Ticket)) ? 'border-primary bg-primary/5' : 'border-cb-border shadow-cb-level-1',
                    SIATC_THEME.TOKENS.RADIUS.CARD
                  )}
                  onClick={() => toggleSelect(String(dev.Ticket))}
                >
          ...
          {devoluciones.length === 0 && !loading && (
            <div className={cn(
              "py-24 bg-card border border-cb-border text-center",
              SIATC_THEME.TOKENS.MASTER_ROUNDNESS
            )}>
          ...
          <div className="flex items-center justify-between px-2">
            ...
            <div className="flex items-center gap-1">
              <button 
                ...
                className={cn(
                  "h-9 px-4 bg-muted/30 border border-border/50 text-[10px] font-black uppercase transition-all hover:bg-muted disabled:opacity-20",
                  SIATC_THEME.TOKENS.RADIUS.BUTTON
                )}
              >
                Anterior
