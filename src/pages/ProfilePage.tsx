...
export default function ProfilePage() {
    ...
    useEffect(() => {
        if (user) {
            // eslint-disable-next-line react-hooks/set-state-in-effect -- sincroniza el formulario local con el usuario del AuthContext al montar/cambiar de usuario
            setFormData({
    ...
    const handleSubmit = async (e: FormEvent) => {
        ...
        setIsSaving(true);
        try {
            const { data: savedProfile } = await apiClient.put<ProfileSaveResponse>('/profile', {
                            ...
                            <div className={layout.AVATAR_CONTAINER}>
                                <div className="relative group">
                                    ...
                                    <button
                                        ...
                                        className={cn(layout.CAMERA_BUTTON, SIATC_THEME.MOBILE.TOUCH_TARGET, "w-11 flex items-center justify-center")}
                                ...
                                <h2 className="mt-4 text-xl font-bold tracking-tight text-cb-text-primary">{user.fullName || user.username}</h2>
                                <p className="text-sm text-primary font-medium">@{user.username}</p>

                                <div className={layout.ROLE_BADGE}>
                                    <Shield className="w-3.5 h-3.5" />
                                    {user.role_name || user.role || t('profile.noRole')}
                                </div>
                            </div>
                        </div>
                            ...
                            <div className={layout.INFO_LIST}>
                                <div className={layout.INFO_ITEM}>
                                    ...
                                    <div className={layout.INFO_ITEM_DETAILS}>
                                        ...
                                        <p className={layout.INFO_ITEM_VALUE}>{user.email || t('profile.info.noEmail')}</p>
                                    </div>
                                </div>
