...
export function AppSwitcher({ currentAppId = 'DEV' }: AppSwitcherProps) {
    ...
    useEffect(() => {
        const fetchApps = async () => {
            try {
                const response = await apiClient.get<Application[]>('/applications?activeOnly=true');
    ...
    // Filter apps:
    // 1. Omit the current app (e.g. DEV)
