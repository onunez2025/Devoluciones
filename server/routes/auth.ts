...
// Con alias a proposito: este fichero no importaba los tipos de express y `Request`/`Response`
// resolvian a los del DOM (fetch), que no tienen ni .status() ni .user.
import type { Request as PeticionExpress, Response as RespuestaExpress } from 'express';
...
router.post('/force-change-password', verifyToken, validateBody(cambioObligatorioSchema), async (req: PeticionExpress, res: RespuestaExpress) => {
    try {
